...
- Use `brainstorming` for open-ended feature ideas or unresolved goals and scope, before `planmate`. For questions, narrowly scoped fixes, and implementation with an already agreed goal and scope, follow the existing workflow. Reuse prior decisions and authorization; ask only when a material decision remains unresolved.
